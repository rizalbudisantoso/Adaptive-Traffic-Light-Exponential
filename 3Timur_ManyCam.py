...
MIN_CYCLE_DURATION_SEC = 12.0          # <<< tambah: biar konsisten seperti West/South
ENABLE_IDLE_TIMEOUT_FACTOR = 0.5      # <<< tambah: enable ON tapi tidak ada hit => matikan
...
def insert_vehicle_count_to_db(count_total, mobil, motor, bis, truk, durasi_efisien, avg_fps):
    """Insert data ke database Way C (East Lane) + log delay + rata-rata FPS ke CSV."""
    ...
    try:
        ...
        sql = """INSERT INTO wayc 
                 (MotorC, MobilC, TruckC, BusC, CountWayC, durasiefisienC) 
                 VALUES (%s, %s, %s, %s, %s, %s)"""
        ...
        csv_dir = "DelayLogg"
        csv_name = "TIMUR_db_delay_log.csv"
        ...
        print("✓ Data tersimpan ke database (East Lane):")
        ...
        print(f"  Delay insert DB (Way C): {delay_sec:.4f} detik")
        print("-" * 50)
...
counting_lines = [
    {"start": (600, 200), "end": (200, 400)},   # Line 0 - Counting
    {"start": (0, 0), "end": (0, 0)},           # Line 1 (disabled)
    {"start": (570, 130), "end": (170, 320)}    # Line 2 - Enable
]
...
def check_direction_right(prev, curr):
...
def processing_thread(args):
    ...
    print(f"\n{'='*50}")
    print("🎥 East Lane Vehicle Counter Started")
    print(f"{'='*50}")
    ...
    while running:
        ...
        # enable idle-timeout: enable ON tapi belum ada hit counting sama sekali
            ...
            for i, line in enumerate(counting_lines):
                ...
                if prev and line_intersect(prev, center, a, b):
                    ...
                    # counting line (index 0)
                        ...
                        if check_direction_right(prev, center):
                            vehicle_counter += 1
        ...
        if enable_active and len(vehicle_timestamps) > 0:
            ...
            # FIX: cek "siklus selesai" dulu (biar tidak keburu reset)
            if prev_win > 0 and ratio_percent < threshold_percent:
                ...
                print("\n" + "=" * 50)
                print("⏱️  SIKLUS SELESAI (East Lane - Ratio Trigger)")
                print("=" * 50)
...
class App:
    def __init__(self, root, args):
        self.root = root
        self.root.title("Vehicle Counter - East Lane (Configurable)")
