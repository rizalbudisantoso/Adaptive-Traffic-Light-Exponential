...
kendaraan_minimal = 10
MIN_CYCLE_DURATION_SEC = 10.0  # durasi minimum agar boleh masuk DB
ENABLE_IDLE_TIMEOUT_FACTOR = 0.5  # <<< setengah dari MIN_CYCLE_DURATION_SEC
...
def insert_vehicle_count_to_db(count_total, mobil, motor, bis, truk, durasi_efisien, avg_fps):
    ...
    try:
        ...
        sql = """INSERT INTO waya 
                 (MotorA, MobilA, TruckA, BusA, CountWayA, durasiefisienA) 
                 VALUES (%s, %s, %s, %s, %s, %s)"""
        ...
        csv_dir = "DelayLogg"
        csv_name = "BARAT_db_delay_log.csv"
        ...
        print(f"  Delay insert DB: {delay_sec:.4f} detik")
        print("-" * 50)
...
counting_lines = [
    {"start": (130, 130), "end": (480, 220)},
    {"start": (480, 220), "end": (600, 120)},
    {"start": (315, 115), "end": (550, 160)}
]
...
def check_direction_line1(prev, curr):
    return curr[1] > prev[1]


def check_direction_line2(prev, curr):
    moving_down = curr[1] > prev[1]
    moving_right = curr[0] > prev[0]
    return moving_down or moving_right
...
def processing_thread(args):
    ...
    conf_thresholds = {'motor': 0.35, 'mobil': 0.45, 'truk': 0.5, 'bis': 0.60}
    ...
    # >>> NEW: watchdog enable idle
    enable_on_time = None        # kapan enable ON
    last_count_time = None       # kapan count pertama terjadi setelah enable ON

    status_queue.put(("RESOLUTION", width, height))
    status_queue.put("RUNNING")

    def reset_cycle_state(reason: str):
        nonlocal cycle_fps_history, enable_on_time, last_count_time
    ...
    def disable_enable_only(reason: str):
        """Khusus untuk kasus enable nyala tapi tidak ada hit sama sekali."""
        ...
        enable_on_time = None
        last_count_time = None
        # NOTE: vehicle_counter tetap 0 di kasus ini, jadi tidak perlu reset siklus penuh.
    ...
    while running:
        ...
        # ===== NEW: enable idle-timeout (sebelum counting juga boleh, tapi di sini aman) =====
        if enable_active and vehicle_counter == 0:
            if enable_on_time is not None:
                idle_timeout = MIN_CYCLE_DURATION_SEC * ENABLE_IDLE_TIMEOUT_FACTOR
                if (current_time - enable_on_time) >= idle_timeout and last_count_time is None:
                    # enable ON, tapi tidak pernah ada hit selama timeout -> matikan enable
                    disable_enable_only(f"Enable ON tanpa hit >= {idle_timeout:.2f}s")
                    # lanjut frame berikutnya supaya tidak menghitung dalam frame yang sama
                    # (opsional)
                    # continue
        ...
        for motor_box in motor_boxes:
            mx1, my1, mx2, my2, mconf, mid, mi = motor_box
            for person_box in person_boxes[:]:
                px1, py1, px2, py2, pconf, pid, pi = person_box
                if calculate_iou((mx1, my1, mx2, my2), (px1, py1, px2, py2)) > 0.1:
        ...
        for track in tracks:
            ...
            for i, line in enumerate(counting_lines):
                ...
                if prev and line_intersect(prev, center, a, b):
                    ...
                    if i == 2 and center[1] > prev[1]:
                        enable_hits.append(time.time())
                        enable_hits[:] = [t for t in enable_hits if time.time() - t <= 1]
                        if len(enable_hits) >= 3 and not enable_active:
                            enable_active = True
                            enable_on_time = current_time
                            last_count_time = None
                            # reset hit-cache supaya enable tidak terus "kepancing"
                            enable_hits = []
                            print("🟢 ENABLE AKTIF (double-hit line 3)")

                    # Line 1
                    elif i == 0 and enable_active and not counted_tracks[track_id][0]:
                        if check_direction_line1(prev, center):
                            vehicle_counter += 1
                            counted_tracks[track_id][0] = True
                            vehicle_timestamps.append(current_time)

                            last_count_time = current_time  # <<< NEW: tanda ada hit
                            ...
                            durasi_saat_ini = last_vehicle_time - first_vehicle_time if first_vehicle_time else 0
                    ...
                    # Line 2
                        ...
                        if check_direction_line2(prev, center):
                            vehicle_counter += 1
                            ...
                            durasi_saat_ini = last_vehicle_time - first_vehicle_time if first_vehicle_time else 0
                            print(f"🚗 Kendaraan #{vehicle_counter} ({class_name}) | Line 2 | Durasi: {durasi_saat_ini:.2f}s")
            ...
            for k in range(1, len(traj)):
                cv2.line(vis_frame, traj[k - 1], traj[k], color, 2)
        ...
        if enable_active and len(vehicle_timestamps) > 0:
            ...
            if prev_win > 0 and ratio_percent == 0.0 and curr_win == 0:
                enable_active = False
                enable_hits = []
                enable_on_time = None
                last_count_time = None
            ...
            if prev_win > 0 and ratio_percent < threshold_percent:
                ...
                print("\n" + "=" * 50)
                print("⏱️  SIKLUS SELESAI (Ratio Trigger)")
                print("=" * 50)
        ...
        for i, line in enumerate(counting_lines):
            color = (0, 255, 0) if i < 2 and enable_active else (255, 0, 0) if i < 2 else (0, 0, 255)
            cv2.line(vis_frame, line["start"], line["end"], color, 3)
...
class App:
    def __init__(self, root, args):
        self.root = root
        self.root.title("Vehicle Counter - West Lane (Configurable)")
